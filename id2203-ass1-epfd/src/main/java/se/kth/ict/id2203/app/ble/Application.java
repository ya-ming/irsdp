...
public final class Application extends ComponentDefinition {
    ...
    private Handler<BallotLeader> handleLeader = new Handler<BallotLeader>() {
		@Override
		public void handle(BallotLeader event) {
			logger.info("Process {} is the leader with ballot number {}", event.getLeader(), event.getBallot().getN());
		}
	};

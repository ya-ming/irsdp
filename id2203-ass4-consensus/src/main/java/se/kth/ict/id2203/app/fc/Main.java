 ...
 */
package se.kth.ict.id2203.app.fc;
...
public class Main extends ComponentDefinition {
	...
	public Main() {
		...
		Component fc = create(Fc.class, new FcInit(self, pi));
		...
		subscribe(handleFault, fc.control());
		subscribe(handleFault, app.control());

		connect(app.required(FloodingConsensus.class), fc.provided(FloodingConsensus.class));
		...
		connect(fc.required(BestEffortBroadcast.class), beb.provided(BestEffortBroadcast.class));
		connect(fc.required(PerfectFailureDetector.class), pfd.provided(PerfectFailureDetector.class));

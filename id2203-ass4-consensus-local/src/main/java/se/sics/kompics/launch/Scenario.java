...
public abstract class Scenario {
	...
	private void tryNew(int id, final String command) {
		System.out.println("Create a new process " + id);
		OldProcessLauncher processLauncher = processes.get(id);
		if (processLauncher == null) {
			int idx = 4;
			long now = System.currentTimeMillis();
			Semaphore semaphore = new Semaphore(0, false);
			Command cmd  = command(id, command);
			processLauncher = createProcess(id, idx++,
					cmd, now, semaphore);
			processes.put(id, processLauncher);

			processLauncher.setProcessCount(processCount);
			processLauncher.setTopologyFile(file.getAbsolutePath());
			processLauncher.start();
		} else {
			System.err.println("Scenario contains process " + id);
			return;
		}
	}
				...
				while (true) {
					try {
						...
						if (line.startsWith("recover")) {
							...
							System.out.println("Received create a new process ");

...
public final class OldProcessLauncher extends Thread {
	...
	private void launchProcess(String commandScript, long delayMs, String action) {
		...
		Integer address = 1044 + id;
		ProcessBuilder processBuilder = new ProcessBuilder("java", "-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=" + address.toString(),
				"-classpath", classpath, log4jProperties, "-Dtopology="
						+ topologyFile, mainClass, "" + id,
				commandScript);

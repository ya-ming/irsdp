...
import se.kth.ict.id2203.components.fc.Fc;
import se.kth.ict.id2203.components.fc.FcInit;
import se.kth.ict.id2203.components.fuc.Fuc;
import se.kth.ict.id2203.components.fuc.FucInit;
...
public class Main extends ComponentDefinition {
	...
	public Main() {
		...
		Component fuc = create(Fuc.class, new FucInit(self, pi));
		...
		subscribe(handleFault, fuc.control());
		subscribe(handleFault, app.control());

		connect(app.required(FloodingUniformConsensus.class), fuc.provided(FloodingUniformConsensus.class));
		...
		connect(fuc.required(BestEffortBroadcast.class), beb.provided(BestEffortBroadcast.class));
		connect(fuc.required(PerfectFailureDetector.class), pfd.provided(PerfectFailureDetector.class));

 ...
 */
package se.kth.ict.id2203.app.hc;
...
public class Main extends ComponentDefinition {
	...
	public Main() {
		...
		Component hc = create(Hc.class, new HcInit(self, pi));
		...
		subscribe(handleFault, hc.control());
		subscribe(handleFault, app.control());

		connect(app.required(HierarchicalConsensus.class), hc.provided(HierarchicalConsensus.class));
		...
		connect(hc.required(BestEffortBroadcast.class), beb.provided(BestEffortBroadcast.class));
		connect(hc.required(PerfectFailureDetector.class), pfd.provided(PerfectFailureDetector.class));
